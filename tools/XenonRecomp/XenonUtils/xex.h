...
{
    be<uint32_t> key;
    be<uint32_t> value;   // union alias 'offset' folded in (identical 4-byte layout)
};
...
inline const void* getOptHeaderPtr(const uint8_t* moduleBytes, uint32_t headerKey)
{
    ...
    for (uint32_t i = 0; i < xex2Header->headerCount; i++)
    {
        ...
        if (optHeader.key == headerKey)
        {
            ...
            {
                return reinterpret_cast<const void *>(reinterpret_cast<uintptr_t>(moduleBytes) + optHeader.value);
            }
        }
    }
