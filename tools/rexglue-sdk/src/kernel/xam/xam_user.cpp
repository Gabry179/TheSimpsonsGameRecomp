...
i32 XamUserGetXUID_entry(u32 user_index, u32 type_mask, mapped_u64 xuid_ptr) {
  ...
  if (user_index < 4) {
    {
      // HAND PATCH: alias all user slots to the single local profile (see
...
u32 XamUserGetSigninState_entry(u32 user_index) {
  ...
  if (user_index < 4) {
    // HAND PATCH: report the single local profile as signed in on EVERY
    // user slot, not just slot 0. The Simpsons Game binds the profile
    // query to the controller's port (observed querying user=1), and with
    // only slot 0 signed in it refuses to save ("not signed into a gamer
    // profile"). Single-player title, so aliasing all slots is safe.
    const auto& user_profile = REX_KERNEL_STATE()->user_profile();
